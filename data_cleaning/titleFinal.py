...
path = '/Users/apple/Desktop/Roberto-Research/TitleExtractor/ORIGINAL TXT list'
count = 0
titles = []
for filename in glob.glob(os.path.join(path, '*.txt')):
	with open(filename,'r') as each:
		...
		with open('/Users/apple/Desktop/Roberto-Research/TitleExtractor/output/data%d__%s' %(count, filename), 'w') as out:
			title = []
			for sentence in sentences:
				if isTitle(sentence):
...
with open('/Users/apple/Desktop/Roberto-Research/TitleExtractor/titles.txt','w') as output:
	...
	for i,title in enumerate(titles):
		if title:
			count += 1
		output.write('\n')
		output.write('Essay%d:     %s: ' % (i,title[1][:-1]))
		output.write('\n')
		for t in title[0]:
			if t and t[-1]!='.':
				output.write(t + '.')
